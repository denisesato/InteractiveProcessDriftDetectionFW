...
def generate_dfg(sub_log, event_data_original_name, w_count):
    ...
    model_path = os.path.join(Info.get_data_models_path(), dfg_path, event_data_original_name)
    ...
    print(f'Salvando {model_path} - {output_filename}')
...
def get_dfg(log_name, window):
    ...
    models_path = os.path.join(Info.get_data_models_path(), dfg_path, log_name)
